...
document.querySelector("#btn1").addEventListener("click", () => {
  ...
  // -> li1 부모 노드 배경색 변경
  li1.parentNode.style.backgroundColor = "skyblue"; // 부모노드가 ul태그임
  ...
  // 4. 원하는 위치(index를 이용)에 존재하는 자식 노드 찾기
  // (요소.childNodes[index])
  // -> #test의 자식 노드 중 3번째 li 태그를 찾아서 배경색 pink로 지정하기
  console.log(test.childNodes[9].style.backgroundColor = "pink");
  ...
  console.log(test.childNodes[12].previousSibling);
  ...
  list[11].append("12345","abc","IOP");

  // 3. 첫 번째 자식으로 추가 ( prepend("내용" 또는 노드 또는 요소 여러개) )
  list[11].prepend("오늘","점심","뭐먹지");

  // 4. 이전/다음 형제로 추가 ( before/after("내용" 또는 노드 또는 요소 여러개) )
  // #test 전/후에 내용 추가
  test.before("이전");
  test.after("이후");
});
...
btn2.addEventListener("click", ()=> {
  ...
  console.log(test2.firstElementChild);     // 1번 li

  // #test2의 마지막 자식 요소 찾기
  console.log(test2.lastElementChild);      // 4번 li
  ...
  // #test2 다음 형제 요소
  console.log(test2.nextElementSibling);      // 버튼 태그
});