...
document.getElementById("inputPno").addEventListener("keyup", (e) => {
  ...
  // 생년월일(6)-고유번호(7) // 특수문자 그 자체를 나타내고 싶다면 \- , \* 등 \를 앞에 붙인다
  const regExp = /^\d{6}\-\d{7}$/;

  if( regExp.test(e.target.value) ) {
    span.innerText = "유효한 주민등록번호입니다";
    span.classList.add("confirm"); // 앞의 CSS 클래스 색상
    span.classList.remove("error");
  }
  else {

    span.innerText = "유효하지 않은 주민등록번호입니다";
    span.classList.add("error"); // 앞의 CSS 클래스 색상
    span.classList.remove("confirm");
  }
});